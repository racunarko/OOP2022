package hr.fer.oop.pred15_Swing.swing1.events.example2;
...
public class InputUserDataPanel extends JPanel{
    ...
    public InputUserDataPanel() {
        ...
        radioPanel.setBorder(BorderFactory.createTitledBorder(null, "Notifications interval?", TitledBorder.LEADING, TitledBorder.TOP));
        add(radioPanel);
        ...
        comboCity = new JComboBox<>(cityArray);
        add(comboCity);
    ...
    public UserData getUserData() {
        ...
        if(rbDaily.isSelected()) {
            data.setEmailType(UserData.EmailType.DAILY);
        } else if(rbMonthly.isSelected()) {
            data.setEmailType(UserData.EmailType.MONTHLY);
        } else {
            data.setEmailType(UserData.EmailType.WEEKLY);
        }
    ...
    public void setUserData(UserData userData) {
        ...
        if(userData.getEmailType() != null) {
            switch (userData.getEmailType()) {
                case DAILY -> rbDaily.setSelected(true);
                case MONTHLY -> rbMonthly.setSelected(true);
                case WEEKLY -> rbWeekly.setSelected(true);
                default -> throw new RuntimeException(
                        "UserData.EmailType is not recognized: type=" + userData.getEmailType());
            }
        } else {
