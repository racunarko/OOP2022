...
public class DirContent {
    public static void main(String[] args) throws IOException {
        ...
        DirectoryStream.Filter<Path> filter = new FilterByExtension("txt", "pdf", "pptx", "java");

        try(DirectoryStream<Path> dirStream = Files.newDirectoryStream(directory, filter)) {
            for (Path path : dirStream) {
                System.out.printf("%s (%s bytes) (%s) \n", path.getFileName().toString(), Files.size(path), Files.getLastModifiedTime(path).toString());
            }
        }
    }
}
